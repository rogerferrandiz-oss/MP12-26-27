...
Abans de donar-vos un contracte real, us farem una formació amb la que us acompanyarem en el primer projecte que fareu amb nosaltres. 

Així aprendreu a:

- Fer servir els tutorials i material del curs de formació.
- Entendreu com fer les coses a Aperture.
```
...
Tenim un contracte fictici, que fem servir per la formació. Ja està resolt en algunes parts perquè tingueu un punt de partida. 

Així ens podem centrar en explicar-vos com es fa un projecte a la nostra consultoria.
```
...
La idea és que feu servir aquest projecte de model per aprendre què heu de fer, quan ho heu de fer i com ho heu de fer.
```
...
El primer pas a qualsevol projecte, és conèixer què necessita el client, per poder-li proposar una solució. Això és una cosa completament nova per a vosaltres que veniu de l'institut. 

Al món laboral, les coses van diferents.
```

```destacat-info
- Un projecte és un problema com els que us ha tocat fer a classe, però que us feu vosaltres l'enunciat.
- Podeu veure l'oferta com fer l'enunciat del problema, amb la dificultat que el projecte us el defineix la necessitat del client.
```

### QUÈ ÉS UNA FITXA DE CLIENT
```bloc-dialeg
avatar: lidia_formal
nom: Lídia García
rol: Recursos Humans i Qualitat · Aperture Holistics Consulting

Tot projecte comença amb una petició d'oferta per part del client. El nostre departament comercial atén les peticions dels possibles clients, les organitza i classifica. Els novells rebeu les més fàcils. 

Això a altres companyies ho haurieu de fer vosaltres i no tindreu opció de triar el nivell. 
```

```destacat-warning
Cada possible client té assignat un codi de client, i tota la documentació que fem (els arxius i la carpeta principal) han de dur expressament aquest codi al nom.
```

### LA VOSTRA PRIMERA FITXA. TECHNOPLAY
```bloc-dialeg
avatar: albert_formal
nom: Albert Serrano
rol: Mentor tècnic · Aperture Holistics Consulting

El client Technoplay és inventat. No existeix. Està fet a mida per poder anar explicant tots els procediments i passos que fem a Aperture (i a totes les empreses una mica serioses) per resoldre els problemes dels clients.
```

```destacat-reading
El primer que heu de fer, és llegir la fitxa del client, per conèixer-lo i entendre què necessita.
```
...
```destacat-info
Tingueu algunes coses en compte:

- El client no sempre sap explicar què necessita ni normalment no té capacitat tècnica per donar tots els detalls que necessitem per saber què ens demana.
- Els comercials només recullen la seva trucada i fan el millor que saben per omplir els buits. Segurament no ho han entès tot. 

**Els tècnics sou vosaltres. La primera feina és llegir atentament la fitxa, i un cop l'heu llegida:**
...
Un cop hem llegit l'oferta i apuntat els dubtes, és el moment de continuar el procediment per a preparar l'oferta. Això passa per omplir el document de requisits.
...
```destacat-info
Aquest document és autoexplicatiu. Porta les instruccions en els diferents apartats.

- Seguiu les instruccions que porta escrites. 
- Quan acabeu, no ha de quedar res de les explicacions al document, i sí que ha de contenir tot el que fa referència al projecte.
- Per omplir-lo segurament necessitareu apuntar i pensar una mica què ens està demanant.
```

### REPASSEM JUNTS EL DOCUMENT DE REQUISITS
```bloc-dialeg
avatar: albert_formal
nom: Albert Serrano
rol: Mentor tècnic · Aperture Holistics Consulting

Un cop heu acabat el document, me l'heu de presentar perquè el validi. Això us assegurarà que algú amb experiència li doni un cop d'ull abans de continuar. 

Sense la meva signatura, el document no està tancat i no podeu continuar. I no us l'aprovaré de qualsevol manera. Us el faré repetir tants cops com calgui fins que estigui segur que ho heu entès.
```
...
Ara amb tota la informació, els dubtes i el guió, ja podeu contactar amb Marc i concertar una primera reunió per conèixer-lo, preguntar tot el que necessiteu. 

Us deixem aquí els tutorials que heu de fer servir per fer tota la feina. Els tindreu a la vostra disposició durant tot el curs. Necessitareu una mica de temps per entendre com funcionen.

Tots els tutorials tenen un vídeo explicatiu.
```
...
No us oblideu de fer l'acta de reunió. Com l'heu de fer, surt al tutorial que ja us hem enllaçat.

A continuació teniu la plantilla d'acta de reunió. També la podeu trobar al directori de plantilles corporatives.
...
Un cop ja heu fet l'acta de la reunió i l'heu enviada al client, ja hauríem de tenir prou informació com per poder-nos posar a fer l'oferta. 
...
```destacat-ideas
**Per fer l'oferta al Marc:**

- És un projecte sobretot de maquinari. Teniu els preus de La Botiga del PC per fer el pressupost.
- Què pot fer que els ordinadors fallin així?

**Per la descomposició de tasques:**

- Recordeu les pràctiques de maquinari. Quan es triga a fer cada tasca de manteniment?
- On puc buscar mesures de rendiment per comparar amb els resultats que em dona el client?

**Quan feu el calendari:**

- Mireu de no superar les 4 setmanes, ni de fer-ho en dues, perquè aleshores el client us ho demanarà en dues i si hi ha problemes, no complirem.
- Compteu que el material físic no és instantani. Arriba dies després de ser demanat. Aquests dies s'han de veure al calendari com a "comanda" i "recepció de comanda".

**Per fer el pressupost:**
