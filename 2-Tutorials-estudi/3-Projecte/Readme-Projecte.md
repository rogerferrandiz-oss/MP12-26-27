...
El que us explicarem aquí no està relacionat directament amb la part tècnica dels projectes. Té més a veure amb la part de la gestió del mateix i que són molt diferents respecte del que coneixeu del cicle.
```

```destacat-warning
**Fer un projecte no és una cursa de velocitat**. No donen premi al que acaba primer. 
- Tens més probabilitat d'acabar malament un projecte fet a corre-cuita que un projecte que segueix les fases i els temps adequats.
- Tot es tracta de fer funcionar allò que el client ha demanat que faci, i ho faci sense errades.
```
...
Com hem vist aquí al tutorial del cicle en V dels projectes, aquests es fan d'una forma organitzada que facilita que es puguin acabar en temps i cost, i que faciliten la resolució dels possibles problemes que puguin aparèixer mentre el fas.
```

```bloc-boto
text: Tutorial del cicle en V d'un projecte
url: https://cv.copernic.cat/mod/book/edit.php?cmid=23673
icona: fa-link
estil: primari
```
...
### DELS REQUISITS AL DISSENY (subcapítol 1)
...
Amb el document de l'oferta i havent de donar un preu i un calendari assumibles, hem hagut de pensar a grans trets quina és la solució al problema que ens planteja el client, els components bàsics, com es connecten entre ells i les seves configuracions.

Seguint la banda esquerra descendent del projecte:
...
### INSTAL·LACIÓ I IMPLEMENTACIÓ DEL PROJECTE (subcapítol 2)
...
Aquesta fase és el cor del projecte. Aquí fem la construcció real del sistema, muntem els components, etc. Fem que el sistema funcioni tal i com el tenim dissenyat.
...
A Aperture tenim projectes que duren setmanes i altres que duren anys. I en tots ells mantenim una comunicació transparent amb el client, que és qui ens paga per resoldre-li el problema. Els clients han d'estar informats en tot moment sobre com està anant l'encàrrec.

Com els projectes dels consultors que comencen solen ser curts, com a mínim farem una reunió de seguiment amb el nostre client per tal que agafeu el costum, i també per mostrar al client que:
- Estem treballant en el projecte. 
- Estem al dia o tenim endarreriments.
- Sabem fer el projecte de forma professional. Confiança.
- Ens importa el seu problema. Fem les preguntes que necessitem per aclarir els dubtes que han sortit.
```

Aquestes reunions es desenvolupen com sempre, seguiu el tutorial de reunions
```bloc-boto
text: Tutorial sobre com fer les reunions
url: https://cv.copernic.cat/mod/book/view.php?id=23449
icona: fa-link
estil: primari
```
...
Un cop hem acabat de muntar el nostre disseny, és el moment de comprovar que tot funciona correctament. La pràctica habitual és separar les proves en diferents nivells, com s'ha explicat al procés de cicle en V [link al tutorial].
...
Si necessiteu saber més sobre les proves, teniu informació aquí
```bloc-boto
text: Tutorial sobre els protocols de proves
url: https://cv.copernic.cat/mod/book/edit.php?cmid=23672
icona: fa-link
estil: primari
```
...
Tancar un projecte també és un petit projecte en sí mateix. I us ho explico ràpidament.
...
Per això, un cop tingueu el projecte llest per fer la reunió d'acceptació amb el client, passareu una auditoria interna per assegurar que:
...
Pot semblar que és molt, però si treballeu bé des del primer dia, superareu sense problemes aquesta auditoria. Seguiu el que se us indica al manual d'estil d'Aperture.
```

```bloc-boto
text: Guia d'estil d'Aperture
url: https://cv.copernic.cat/mod/book/view.php?id=23462
icona: fa-link
estil: primari
```
